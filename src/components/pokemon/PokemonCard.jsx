...
function PokemonCard({ url }, ref) {
  ...
  return (
    <div
      className="flex shrink-0 grow-0 basis-1/4 flex-col items-center justify-center px-4"
      ref={ref}
    >
      <div
        className="align-center z-10 w-20 translate-y-8 cursor-pointer"
        onClick={() => selectPokemon(pokemonData.id)}
      >
        <PokemonImage id={id} name={pokemonData.name} />
      </div>
      <div
        className="z-0 flex size-full grow cursor-pointer flex-col items-center justify-center gap-y-4 rounded-xl bg-white pb-4 pt-12"
        onClick={() => selectPokemon(pokemonData.id)}
      >
